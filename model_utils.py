...
def get_parameters(models):
    ...
    parameters = []
    if isinstance(models, list):    # in case someone write list
        for model in models:
            parameters += get_parameters(model)
    elif isinstance(models, dict):  # if we have two models we do this 
        for model in models.values():
            parameters += get_parameters(model)
    else: # models is actually a single pytorch model
        parameters += list(models.parameters())  # model.parameters() return a generator and we turn it to list
    return parameters

def get_optimizer(hparams, models):
    eps = 1e-8
    parameters = get_parameters(models)   # train parameters have nothing to do with hparam
    if hparams.optimizer == 'sgd':
        optimizer = SGD(parameters, lr=hparams.lr,    # input of torch.optim.SGD or Adam could be list (dicts)
